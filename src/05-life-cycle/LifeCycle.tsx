      ...
      <section className="flex flex-col m-5 gap-10 py-25">
        ...
        <article className="">
          ...
          <div className="flex gap-3 m-4 items-center">
            <span className="px-5 py-0.5 rounded-2xl bg-pink-400/40 text-white">
              Will 접두사 메서드
            </span>
            <span className="text-pink-300">어떤 작업을 작동하기 전에 실행되는 메서드</span>
          </div>
          <div className="flex gap-3 m-4 items-center">
            <span className="px-5 py-0.5 rounded-2xl bg-blue-400/40 text-white">
              Did 접두사 메서드
            </span>
...
export function GetSnapshotBeforeUpdate() {
  return (
    <div className="flex flex-col justify-center gap-3">
      ...
      <div className="w-120">
        <CodeBlock
          content="getSnapshotBeforeUpdate(
    if(prevState.array !== this.state.array) {
      ...
      <div>
        <div className="flex flex-col">
          <span className="whitespace-pre ">
            리액트 v16.3 이후 만들어졌으며, render의 결과물이 브라우저에 반영되기 전에 호출되는
            메서드
          </span>
          <div className="flex">
  ...
  return (
    <div className="flex flex-col justify-center gap-3">
      ...
      <div>
        <div className="flex flex-col">
          <span className="whitespace-pre ">
            리렌더링을 완료한 뒤 실행하는 메서드로, 업데이트가 끝난 직후이기 때문에 DOM 관련 처리를
            해도 무방함
          </span>
          <span className="whitespace-pre ">
            여기선 prevProps, prevState를 사용해 컴포넌트의 이전 데이터에 접근할 수 있음
          </span>
          <div className="flex items-center">
  ...
  return (
    <div className="flex flex-col justify-center gap-3">
      ...
      <div>
        ...
        <span className="whitespace-pre text-white">componentDidMount</span>
        <span className="whitespace-pre ">
          에서 등록한 이벤트나 타이머, 직접 생성한 DOM이 있을 경우 여기서 제거 작업을 해야 함
        </span>
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div>
        <div className="flex flex-col">
          <span className="whitespace-pre">
            리액트 v16에서 도입되었으며 컴포넌트 렌더링 도중 에러 발생 시 어플리케이션의 중단 없이
            오류 UI를 보여 주도록 가능
          </span>
          <span className="whitespace-pre">
            error는 파라미터에 어떤 에러가 발생헀는지 알려 주며, info 파라미터는 어디에 있는
            코드에서 오류가 발생했는지에 대한 정보를 제공
          </span>
  ...
  return (
    <div className="flex max-w-1/3 justify-between items-center mx-auto relative pt-50">
      {arr.map(({ title, desc }, index) => (
        <div
          key={title}
          className={`${index === 1 ? 'absolute top-0 left-[31.5%]' : ''} flex flex-col items-center gap-2 justify-center`}
        >
          {index === 1 && (
            <>
          ...
          <div
            className={`w-30 h-15 bg-blue-400/40 rounded flex items-center justify-center relative z-10`}
          >
            {index === 0 && (
              <>
    ...
    <div className="flex-1 py-10">
      ...
      <div className="flex flex-col py-3">
        ...
        <div className="my-5">
          ...
          <MountMethod
            title="constructor"
            desc="컴포넌트를 새로 만들 때마다 호출되는 클래스 생성자 메서드입니다"
            isArrow={true}
          />
          <MountMethod
            title="getDerivedStateFromProps"
            desc=" props에 있는 값을 state에 넣을 때 사용하는 메서드입니다"
            isArrow={true}
          />
          <MountMethod
            title="render"
            desc="우리가 준비한 UI를 렌더링하는 메서드입니다"
            isArrow={true}
          />
          <MountMethod
            title="componentDidMount"
            desc=" 컴포넌트가 웹 브라우저 상에 나타난 후 호출하는 메서드입니다"
            isArrow={false}
          />
        </div>
      </div>
    ...
    <div className="flex-1 py-10">
      ...
      <div className="flex flex-col py-3">
        ...
        <div className="my-5">
          ...
          <UpdateMethod
            title="getDerivedStateFromProps"
            desc="마운트 과정에서도 호출되며 업데이트가 시작하기 전에도 호출되는데, props의 변화에 따라 state의 값에도 변화를 주고 싶을 때 사용한다"
            isArrow={true}
          />
          <UpdateMethod
            title="shouldComponentUpdate"
            desc={`컴포넌트가 리렌더링을 해야 할 지 말아야 할 지 결정하며 true 시 계속 실행, false 시 작업을 중지 (true: 리렌더링 O / false: 리렌더링 X) \n특정 함수에서 forceUpdate 함수를 호출한다면 이 과정을 생략하고 바로 render 함수 호출`}
            isArrow={true}
          />
          <UpdateMethod
            title="true 반환 시 render 호출, false 반환 시 여기서 작업 취소"
            desc=""
            isArrow={true}
            isAction={true}
          />
          ...
          <UpdateMethod
            ...
            desc="컴포넌트 변화를 DOM에 반영하기 직전에 호출하는 메서드입니다"
            isArrow={true}
          />
          <UpdateMethod
            title="웹 브라우저상의 실제 DOM 변화"
            desc=""
            isArrow={true}
            isAction={true}
          />
          <UpdateMethod
            ...
            desc="컴포넌트의 업데이트 작업이 끝난 후 호출하는 메서드입니다"
            isArrow={false}
          />
        </div>
      </div>
      ...
      <div className="flex flex-col py-3">
        ...
        <div className="my-5">
          ...
          <UnmountMethod
            ...
            desc="컴포넌트가 웹 브라우저 상에서 제거되기 직전에 호출되는 메서드입니다"
            isArrow={false}
          />
        </div>
      </div>
  ...
  return (
    <div className="flex-1 flex flex-col items-start justify-center px-3 py-1 gap-1">
      <div className="w-full flex items-center gap-3 ">
        <span className="min-w-50 px-3 py-2 rounded border border-amber-50/30 text-center text-sm  bg-pink-500/10">
          {title}
        </span>
        <span className="size-full px-7 text-white/80 text-sm ">{desc}</span>
      </div>
      <div className="min-w-50 flex items-center justify-center">{isArrow && <Arrow />}</div>
    </div>
  )
}
...
export function UnmountMethod({
  ...
  return (
    <div className="flex-1 flex flex-col items-start justify-center px-3 py-1 gap-1">
      <div className="w-full flex items-center gap-3 ">
        <span className="min-w-50 px-3 py-2 rounded border border-amber-50/30 text-center text-sm  bg-yellow-500/10">
          {title}
        </span>
        <span className="size-full px-7 text-white/80 text-sm ">{desc}</span>
      </div>
    ...
    <div className="flex-1 flex flex-col items-start justify-center px-3 py-1 gap-1">
      <div className="w-full flex items-center gap-3 ">
        {isFirst ? (
          ...
          <div
            className={`${!isAction && 'border bg-blue-500/10 border-amber-50/30'} w-[350px] px-3 py-2 rounded   text-center text-sm  flex items-center justify-center `}
          >
            {title}
          </div>
        )}
      ...
      <div className="w-[350px] flex items-center justify-center">{isArrow && <Arrow />}</div>
    </div>
  )
}
