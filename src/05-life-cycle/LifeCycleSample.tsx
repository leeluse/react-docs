...
class LifeCycleSample extends Component<Props, State> {
  ...
  static getDerivedStateFromProps(nextProps: Props, prevState: State) {
    console.log(
      '🔍 [Mount & Update Phase] getDerivedStateFromProps - Props 데이터를 State와 동기화 시도',
      { nextProps, prevState },
    )
    if (nextProps.color !== prevState.color) {
      console.log(
        `   ➡️ props 색상(${nextProps.color})과 state 색상(${prevState.color})이 다르므로 state를 동기화합니다.`,
      )
  ...
  shouldComponentUpdate(nextProps: Props, nextState: State) {
    ...
    console.log(
      `🤔 [Update Phase] 1. shouldComponentUpdate - 리렌더링 여부 판단 (결과: ${shouldUpdate})`,
      {
  ...
  componentWillUnmount() {
    console.log(
      '🧹 [Unmount Phase] componentWillUnmount - 컴포넌트가 DOM에서 제거되기 직전 정리 작업 실행',
    )
  }
  ...
  getSnapshotBeforeUpdate(prevProps: Props) {
    console.log(
      '📸 [Update Phase] 2. getSnapshotBeforeUpdate - 브라우저 DOM 반영 직전의 DOM 스타일 색상 캡처',
    )
  ...
  componentDidUpdate(prevProps: Props, prevState: State, snapshot: string | null) {
    console.log(
      '📢 [Update Phase] 3. componentDidUpdate - 실제 브라우저 DOM 업데이트 및 리렌더링 완료',
      {
        prevProps,
        prevState,
        snapshot,
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col border-l-2 pl-3 my-2 border-slate-500">
          <p className="text-sm">
            State Color (Derived):{' '}
            <span className="font-mono text-white px-1.5 py-0.5 rounded bg-black/30">
              {this.state.color}
            </span>
          </p>
          <p className="text-sm">
            State Number: <span className="font-semibold">{this.state.number ?? 0}</span>
          </p>
        </div>
        ...
        <p className="text-xs text-amber-400/80 mt-3">
          * number가 10으로 나눈 나머지가 4인 경우 (예: 4, 14...) shouldComponentUpdate가 false를
          반환하여 렌더링이 되지 않습니다.
        </p>
      </div>
    )
